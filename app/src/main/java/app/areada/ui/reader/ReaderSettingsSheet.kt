    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            if (showReadingControls) {
                ...
                SettingsControlSpacer()
                SettingsSection(title = stringResource(R.string.page_turn_effect)) {
                    SegmentedSettingGrid(
                        items = ReaderPageTurnEffect.entries,
                        selected = preferences.pageTurnEffect,
                        label = { effect -> effect.displayLabel() },
                        onSelect = { effect ->
                            onPreferencesChange(preferences.copy(pageTurnEffect = effect))
                        },
                    )
                }
                SettingsControlSpacer()
